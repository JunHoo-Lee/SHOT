...
class MLBaseClass(object):
    ...
    @abc.abstractmethod
    def shot_loss(self, x: torch.Tensor, y: torch.Tensor, adapted_hyper_net: typing.Union[torch.Tensor, higher.patch._MonkeyPatchBase], model: dict) -> torch.Tensor:
        """Calculate the validation loss to update the meta-paramter
        ...
        try:
            for epoch_id in range(self.config['resume_epoch'], self.config['resume_epoch'] + self.config['num_epochs'], 1):
                ...
                for eps_count, eps_data in enumerate(train_dataloader):
                    ...
                    # -------------------------
                    approx_adapted_hyper_net = self.approx_adaptation(x=x_t, y=y_t, model=model) if self.config['SHOT'] else None
                    loss_v = self.validation_loss(x=x_v, y=y_v, adapted_hyper_net=adapted_hyper_net, model=model, approx_hyper_net=approx_adapted_hyper_net)
