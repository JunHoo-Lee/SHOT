...
class Maml(MLBaseClass):
    ...
    def approx_adaptation(self, x: torch.Tensor, y: torch.Tensor, model: dict) -> higher.patch._MonkeyPatchBase:
        """See MLBaseClass for the description"""
        # convert hyper_net to its functional form
        f_hyper_net = higher.patch.monkeypatch(
            module=model["hyper_net"],
        ...
        for _ in range(num_inner):
            ...
            new_q_params = []
            for param, grad in zip(q_params, grads):
                new_q_params.append(higher.optim._add(tensor=param, a1=-self.config['inner_lr'] * self.config['num_inner_updates'] / num_inner, a2=grad))
    ...
    def validation_loss(self, x: torch.Tensor, y: torch.Tensor, adapted_hyper_net: higher.patch._MonkeyPatchBase, model: dict, approx_hyper_net=None) -> torch.Tensor:
        """See MLBaseClass for the description"""
        ...
        if self.config['SHOT']:
            assert approx_hyper_net is not None
            one_step_logits = self.prediction(x=x, adapted_hyper_net=approx_hyper_net, model=model)
            # one_step_loss = F.kl_div(F.log_softmax(one_step_logits, dim=1), F.softmax(logits, dim=1).detach())
            one_step_loss = self.config['loss_function'](one_step_logits, logits.detach())
            loss += self.config['SHOT_lr'] * one_step_loss
